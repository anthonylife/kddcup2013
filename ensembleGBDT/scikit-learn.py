...
def main():
    ...
    if MODEL_NUM == 1:
        ...
        classifier = RandomForestClassifier(n_estimators=50,
                                        ...
                                        random_state=1)
        tc_wfd = open(paths['tc_RF'], 'w')
        td_wfd = open(paths['td_RF'], 'w')
        v_wfd = open(paths['v_RF'], 'w')
    elif MODEL_NUM == 5:
        classifier = GradientBoostingRegressor(loss='ls',
                                        learning_rate=0.1,
                                        n_estimators=500,
                                        max_depth=3)
        tc_wfd = open(paths['tc_GBM'], 'w')
        td_wfd = open(paths['td_GBM'], 'w')
        v_wfd = open(paths['v_GBM'], 'w')
    else:
        sys.exit(1)
