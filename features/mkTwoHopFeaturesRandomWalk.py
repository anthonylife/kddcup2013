...
def mk_twohop_coauthor(author_author_dict, paper_author_dict, pairs):
    features = [[-1, -1, -1, -1] for i in range(len(pairs))]
    for i, pair in enumerate(pairs):
        ...
        for authorid in paper_author_dict[pair[1]]:
            ...
            # 1-hop random walk simulation
            if len(author_author_dict[authorid]) != 0 and len(author_author_dict[0]) != 0:
                features[i][1] +=\
                    len(author_author_dict[authorid]&author_author_dict[pair[0]])*1.0/\
                    (len(author_author_dict[authorid])*len(author_author_dict[pair[0]]))
        features[i][2] = features[i][0]*1.0/(len(paper_author_dict[pair[1]])-1)
        features[i][3] = features[i][1]*1.0/(len(paper_author_dict[pair[1]])-1)
...
def main():
    ...
    tc_doc = paths["tc_6th_doc"]
    td_doc = paths["td_6th_doc"]
    v_doc  = paths["v_6th_doc"]

    # loading instances
    print "Loading labeled instances."
    tc_features = [ins[:78] for ins in csv.reader(open(tc_doc))]
    td_features = [ins[:78] for ins in csv.reader(open(td_doc))]
    v_features = [ins[:78] for ins in csv.reader(open(v_doc))]
