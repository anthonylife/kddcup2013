...
def load_year_info(author_table1, author_table2, author_table3, author_table4,\
        author_table5, author_table6):
    author_paper_year = defaultdict(dict)
    author_year_cnt = defaultdict(dict)
    paper_author = defaultdict(set)
    paper_affi = defaultdict(set)
    affi_paper = defaultdict(set)
    paper_conf = defaultdict(set)
    conf_paper = defaultdict(set)
    paper_jour = defaultdict(set)
    jour_paper = defaultdict(set)
    author_affi = defaultdict(set)
    ...
    author_list4 = [entry for entry in csv.reader(open(author_table4))]
    author_list5 = [entry for entry in csv.reader(open(author_table5))]
    author_list6 = [entry for entry in csv.reader(open(author_table6))]
    ...
    '''for entry in author_list3[1:]:
        if entry[1] != None:
            paper_affi[entry[0]].add(entry[1])
            affi_paper[entry[1]].add(entry[0])

    for entry in author_list4[1:]:
        if entry[1] != 0:
            paper_conf[entry[0]].add(entry[1])
            conf_paper[entry[1]].add(entry[0])

    for entry in author_list5[1:]:
        if entry[1] != 0:
            paper_jour[entry[0]].add(entry[1])
            jour_paper[entry[1]].add(entry[0])

    for entry in author_list6[1:]:
        if entry[1] != None:
            author_affi[entry[0]].add(entry[1])
    '''
    return (author_paper_year, author_year_cnt, paper_author, paper_affi,\
            affi_paper, paper_conf, conf_paper, paper_jour, jour_paper,\
            author_affi)


def make_year_feature(author_paper_year, author_year_cnt, paper_author,\
        paper_affi, affi_paper, paper_conf, conf_paper, paper_jour,\
        jour_paper, author_affi, pairs):

    time_fea_num = 19       # Number of features, which should be specified.
    ...
    for i, pair in enumerate(pairs):
        ...
        ## features of target author's bias
        ...
        if first_year == -1:
            ...
            #if author_paper_year[pair[0]][pair[1]] > 1900 and author_paper_year[]
        ...
        ## features of author-affiliation pair
    return features
...
def make_length_year_feature(author_paper_year, paper_author, pair):
    authors = paper_author[pair[1]]
    length_year_coauthor = [0 for i in range(len(authors))]
    target_year = author_paper_year[pair[0]][pair[1]]

    if len(authors) > 1:
        for i, author in enumerate(authors):
            if author == pair[0]:
                length_year_coauthor[i] = -1
            else:
                max_year = 0
                tag = False
                for paper in author_paper_year[pair[0]].keys():
                    if author in paper_author[paper]:
                        source_year = author_paper_year[pair[0]][paper]
                        if source_year > 1900 and source_year <= 2013:
                            if target_year > 1900 and target_year <= 2013:
                                length_year = target_year - source_year
                                if length_year >= max_year:
                                    max_year = length_year
                                    tag = True
                if tag:
                    length_year_coauthor[i] = max_year
                else:
                    length_year_coauthor[i] = -1
        length_year_coauthor = [length_year for length_year in length_year_coauthor if length_year >= 0]
        if len(length_year_coauthor) < 1:
            return (-1, -1, -1)
        else:
            count = sum(length_year_coauthor)
            max_length = max(length_year_coauthor)
            min_length = min(length_year_coauthor)
            return (count, max_length, min_length)
    else:
        return (-1, -1, -1)
...
def main():
    ...
    (author_paper_year, author_year_cnt, \
            paper_author, paper_affi, affi_paper, paper_conf, conf_paper,\
            paper_jour, jour_paper, author_affi)=load_year_info(paths['author_paper_year'],\
            paths['author_year_cnt'], paths['paper_affi'],\
            paths['paper_conf'], paths['paper_jour'], paths['author_affi'])

    postr_year_fea = make_year_feature(author_paper_year, author_year_cnt,\
            paper_author, paper_affi, affi_paper, paper_conf, conf_paper,\
            paper_jour, jour_paper, author_affi, [entry[0:2] for entry in postr_ins])
    ...
    negtr_year_fea = make_year_feature(author_paper_year, author_year_cnt,\
            paper_author, paper_affi, affi_paper, paper_conf, conf_paper,\
            paper_jour, jour_paper, author_affi, [entry[0:2] for entry in negtr_ins])
    ...
    vali_year_fea = make_year_feature(author_paper_year, author_year_cnt,\
            paper_author, paper_affi, affi_paper, paper_conf, conf_paper,\
            paper_jour, jour_paper, author_affi, [entry[0:2] for entry in vali_ins])
